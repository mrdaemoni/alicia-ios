...
All routes require the existing iOS token. GET /api/body projects private state and current read-only bridge evidence. POST /api/body records a typed explicit event. GET /api/body/source?id=...&offset=... reads five exact indexed passages per page by catalog ID, never an arbitrary path. POST /api/body/ask requests optional local inference. JSON is no-store and the phone uses an ephemeral session for these routes.
